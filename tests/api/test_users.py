...
class TestUserFilterParams:
    ...
    async def seed_users(self, get_test_session) -> list[User]:
        users = [
            User(name="Alice Smith", email="alice@test.com", password="pwd", role=UserRole.CLIENT, birth_date=datetime.date(1990, 1, 1)),
            User(name="Bob Jones", email="bob@test.com", password="pwd", role=UserRole.DOCTOR, birth_date=datetime.date(1985, 5, 10)),
            User(name="Charlie Brown", email="charlie@test.com", password="pwd", role=UserRole.CLIENT, birth_date=datetime.date(2000, 12, 12)),
            User(name="Admin Boss", email="admin_user@test.com", password="pwd", role=UserRole.ADMIN),
        ]
    ...
    async def test_get_users_pagination_and_admin_exclusion(self, ac, seed_users, fake_get_admin_user):        
    ...
    async def test_get_users_filters(self, ac, seed_users, fake_get_admin_user, query_params, expected_names):
...
class TestUserById:
    ...
    async def test_get_user_by_id_not_found_error(self, ac, persisted_user, fake_get_admin_user):        
...
class TestUpdatrePassword:
    async def test_update_user_password(self, ac, fake_get_current_user, get_test_session): 
        update_data = {
